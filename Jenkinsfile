pipeline {
    ...
    stages {
        ...
        stage('Deploy to Docker Host') {
          steps {
            sh    'docker -H tcp://54.221.56.147:2375 stop masterwebapp1 || true'
            sh    'docker -H tcp://54.221.56.147:2375 run --rm -dit --name masterwebapp1 --hostname masterwebapp1 -p 8000:80 muralinikkala/pipelinetestmaster:${BUILD_NUMBER}'
            }
        }
        ...
        stage('Check WebApp Rechability') {
          steps {
          sh 'sleep 10s'
          sh ' curl http://54.221.56.147:8000'
          }
        }
